...
import numpy as np
import time
from mmse  import  eval_modelmmse
...
with tf.device(gpu_tag):
    ...
    for trial in range(n_trials): # for each trial
        ...
        if model_type == "GNCN_t1":
            ...
            agent = GNCN_PDH(args)
        eta_v  = eta # set up optimization process
        opt = tf.keras.optimizers.Adam(learning_rate=eta_v)
        ...
        print("evaluating model's performance on the dev set using Masked Mean Error (MME)")
        eval_modelmmse(agent, dev_set, mask_side="left", verbose=True)
